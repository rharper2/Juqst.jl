...
def text(ax,x,y,textstr,plot_params,box=False):
    ...
    if box:
        ...
        bbox=False
        ax.text(x,y,textstr,color='k',ha='center',va='center',size=fontsize)
    #ax.text(x,y,textstr,color='k',ha='center',va='center',bbox=bbox,size=fontsize)
...
def make_schedule(gates):
    ...
    for gate in gates:
        nos = [int(s) for s in re.findall(r'\d+', ' '.join(gate[1:])) if s.isdigit()]
        qubits =set(range(min(nos),max(nos)+1))
